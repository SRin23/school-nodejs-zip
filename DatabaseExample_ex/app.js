...
function connectDB(){
  ...
  MongoClient.connect(databaseUrl, function(err, db){
    ...
    //mongo://127.0.0.1 / database
    console.log('데이터베이스에 연결되었습니다. : ' + databaseUrl);

    //database 변수에 할당
    //database = db;  //옛날버전
    database = db.db('local'); //이렇게 해야 오류가 안남
  })
}
    ...
    if(database){
      authUser(database, paramId, paramPassword, function(err, docs){
        ...
        if(docs){
          ...
          res.write('<div><p>아이디와 패스워드를 다시 확인하십시요.</p></div>');
          res.write("<br><br><a href='/public/login.html'>다시 로그인 하기</a>");
          res.end();
        } //end -if(docs)
...
var authUser = function(database, id, password, callback){
  console.log('autoUser호출됨 : ' + id + ', ' + password);
  ...
  users.find({"id" : id, "password": password}).toArray(function(err, docs){
    if(err){  //에러 발생시 콜백 함수를 호출하면서 에러 객체 전달
      callback(err, null);
      return;
    }
