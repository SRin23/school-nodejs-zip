...
function createUserSchema(){
	...
	UserSchema = mongoose.Schema({
		...
		hashed_password:{type:String, unique:true,'default':""},
		salt:{type:String, required:true},
		name:{type:String, index:'hashed'},
		age:{type:Number, 'default' : -1},
		create_at : {type:Date, index:{unique:false}, 'dafualt' : Date.now},
		update_at : {type:Date, index:{unique:false}, 'dafualt' : Date.now}
	});
	...
	console.log('UserSchema 정의함.'); //a2

	// User 모델 정의
	UserModel = mongoose.model("users3", UserSchema);
	console.log('users3 정의함') //a3
}
